...
def load_images(folder_path, force_rotate=False):
    """加载图像"""
    ...
    for f in files:
        ...
        if img is not None:
            ...
            if force_rotate:
                ...
                print(f"✓ {os.path.basename(f)} - {original_shape[1]}x{original_shape[0]} → {img.shape[1]}x{img.shape[0]} (强制旋转)")
            else:
...
def main():
    ...
    try:
        images, files = load_images(IMAGE_FOLDER, force_rotate=FORCE_ROTATE)
    except Exception as e:
        print(f"❌ 错误: {e}")
        return
