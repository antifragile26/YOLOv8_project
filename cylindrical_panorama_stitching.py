...
def get_exif_data(image_path):
    ...
    try:
        ...
        if focal_mm:
            ...
            # 根据方向判断
            if orientation in [5, 6, 7, 8]:  # 旋转了 90 或 270 度
                width, height = height, width
                print(f"⚠ 检测到图片旋转 (Orientation={orientation})")

            focal_px = focal_mm * (width / sensor_width)
            print(f"✓ 焦距: {focal_mm:.1f}mm → {focal_px:.0f}px")
            return focal_px, orientation
...
def align_neighboring_pairs(warped_images):
    ...
    for i in range(len(warped_images) - 1):
        ...
        print(
            f"  {i}↔{i+1}: dx={trans[0]:.1f}, dy={trans[1]:.1f}, 内点={inliers}, 误差={err:.2f}px"
        )
...
# ==================== 主程序 ====================
def main():
    parser = argparse.ArgumentParser(description='圆柱全景图拼接程序')
    parser.add_argument('--input', '-i', type=str, required=True,
                        help='输入图像文件夹路径')
    parser.add_argument('--output', '-o', type=str, default='panorama.jpg',
                        help='输出全景图文件名 (默认: panorama.jpg)')
    parser.add_argument('--focal', '-f', type=float, default=None,
                        help='手动指定焦距(像素), 如果EXIF读取失败则使用此值')
    parser.add_argument('--sensor-width', '-s', type=float, default=6.17,
                        help='传感器宽度(mm), 默认: 6.17 (1/2.3" 传感器)')
